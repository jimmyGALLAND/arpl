...
function getLKMs() {
    ...
    rm -f "${CACHE_FILE}"
    TAG=`curl -s "https://api.github.com/repos/${REPO_ARPL}/redpill-lkm/releases/latest" | grep -oP '"tag_name": "\K(.*)(?=")'`
    STATUS=`curl -w "%{http_code}" -L "https://github.com/${REPO_ARPL}/redpill-lkm/releases/download/${TAG}/rp-lkms.zip" -o "${CACHE_FILE}"`
...
function getAddons() {
    ...
    TAG=`curl -s https://api.github.com/repos/${REPO_ARPL}/arpl-addons/releases/latest | grep -oP '"tag_name": "\K(.*)(?=")'`
    STATUS=`curl -w "%{http_code}" -L "https://github.com/${REPO_ARPL}/arpl-addons/releases/download/${TAG}/addons.zip" -o "${CACHE_FILE}"`
    echo "TAG=${TAG}; Status=${STATUS}"
    [ ${STATUS} -ne 200 ] && exit 1
    rm -rf "${DEST_PATH}"; mkdir -p "${DEST_PATH}"
    # Install Addons
...
function getModules() {
    ...
    rm -f "${CACHE_FILE}"
    TAG=`curl -s https://api.github.com/repos/${REPO_ARPL}/arpl-modules/releases/latest | grep -oP '"tag_name": "\K(.*)(?=")'`
    STATUS=`curl -w "%{http_code}" -L "https://github.com/${REPO_ARPL}/arpl-modules/releases/download/${TAG}/modules.zip" -o "${CACHE_FILE}"`
    echo "TAG=${TAG}; Status=${STATUS}"
    [ ${STATUS} -ne 200 ] && exit 1
    # Unzip Modules
    rm -rf "${DEST_PATH}"; mkdir -p "${DEST_PATH}"
